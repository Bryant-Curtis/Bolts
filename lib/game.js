(function() {
  ...
  // Repeat this "step" in each frame
  ...
  Game.prototype.onCollision = function(bolt, player) {
    ...
    // Add a HIT effect to the player -
    // Make the background go white for a moment,
    // with the player being drawn like he jumped
    this.drawHit(player);
    if (player.life > 1) {
      player.life -= 1;
    } else {
  ...
  Game.prototype.drawHit = function(player) {
    currentScore = this.score
    playerHitToken = setInterval(this.playerHit.bind(this, player), 500);
  };

  Game.prototype.playerHit = function(player) {
    if (player.life == 2 && this.score - currentScore < 2000) {
      player.color = "orange";
      player.radius = 15;
    } else if (player.life === 1 && this.score - currentScore < 5000) {
      player.color = "red";
      player.radius = 10;
    } else {
      this.playerRecovers(player);
    }
  };

  Game.prototype.playerRecovers = function(player) {
    clearInterval(playerHitToken);
    player.color = "blue";
  }
  ...
  // The player wraps across the screen.
  ...
  // Collect all objects into one array to draw to the screen
  ...
  // Draw the background, score and all objects to screen
  ...
  Game.prototype.draw = function (ctx) {
    ...
    // Keep the this.drawScore BEFORE the this.allObjects.
    // It keeps the game running faster
